// Package srp calculates the responses to cognito srp challenges
// to authenticate client and get tokens
//
// This is de facto utility package for main Auth package
//
// Is is de facto copy-paste from https://github.com/AlexRudd/cognito-srp
// (which is based on capless warrant: https://github.com/capless/warrant)
// The only changed thing is that I removed the dependency to aws sdk
// and aws cognitoidentityprovider because for this package they are not needed.
...
// RespondToAuthChallengeInput is added in spite of importing the type from
// aws sdk for cognitoidentityprovider
...
func (csrp *CognitoSRP) PasswordVerifierChallenge(challengeParms map[string]string, ts time.Time) (*RespondToAuthChallengeInput, error) {
	...
	// ClientId value was changed from aws.String to pointer to string
	// to remove aws sdk dependency
